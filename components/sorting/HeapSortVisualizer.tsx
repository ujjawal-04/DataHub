...
export default function HeapSortVisualizer() {
  ...
  const heapSort = async () => {
    ...
    for (let i = n - 1; i > 0; i--) {
      ...
      setSteps([...currentSteps, `Step ${currentSteps.length + 1}: ${arr.join(', ')}`])
      setArray([...arr])
      await new Promise(resolve => setTimeout(resolve, 100))
  ...
  return (
    <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-6 rounded-lg shadow-lg">
      <div className="mb-4">
        ...
        <button onClick={heapSort} disabled={sorting} className="bg-green-600 text-white hover:bg-green-500 px-4 py-2 rounded-lg transition-all ml-2">Sort</button>
      </div>
